...
from utils.ldm_utils import clear_color
...
class DDIMSampler(object):
    ...
    # def ddecode(self, x_latent, cond=None, t_start=50, temp=1,
    ...
    #         t_start: 起始时间步
    #         temp: 温度参数
    #         unconditional_guidance_scale: 无条件引导尺度
    #         unconditional_conditioning: 无条件引导信息
    #         use_original_steps: 是否使用原始时间步
    #     """
    #     try:
    #         # 设置时间步
    #         timesteps = np.arange(self.ddpm_num_timesteps) if use_original_steps else self.ddim_timesteps
    ...
    #             try:
    #                 index = total_steps - i - 1
    #                 ts = torch.full((x_latent.shape[0],), step,
    #                                 device=x_latent.device, dtype=torch.long)
    #
    #                 # 调用p_sample_ddim并正确处理返回值
    #                 p_sample_result = self.p_sample_ddim(
    #                     x_dec,
    #                     cond,
    #                     ts,
    #                     index=index,
    ...
    #                     x_dec = p_sample_result[0]  # 只取第一个返回值
    #                     if len(p_sample_result) > 1:
    #                         intermediates.append(p_sample_result[1])
    #                 else:
    #                     x_dec = p_sample_result
    #
    #                 # 更新进度条描述
    #                 iterator.set_postfix({
    #                     'step': step,
    #                     'mean': x_dec.mean().item(),
    #                     'std': x_dec.std().item()
    #                 })
    #
